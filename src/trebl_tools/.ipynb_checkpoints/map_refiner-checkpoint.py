...
class MapRefiner:
    ...
    def create_unique_target(self, previous_table):
        ...
        print(
            f"Filtering fixed so that at least {frac*100:.0f}% of reads "
            f"come from the most abundant target..."
        )
    
        base_table = self._prefixed(previous_table)
        filter_tables = []
    
        # Step 1: for each (key, target) mapping, keep only dominant target per key
        for i, (key_cols, target_cols) in enumerate(self.column_pairs):
            ...
            query = f"""
                ...
                ranked AS (
                    SELECT
                        *,
                        SUM(cnt) OVER (PARTITION BY computed_key) AS total_cnt,
                        ROW_NUMBER() OVER (
                ...
                FROM ranked
                WHERE rn = 1
                  AND CAST(cnt AS FLOAT) / total_cnt >= {frac};
            """
    
            self.con.execute(query)
    
        # Step 2: intersect dominant (key, target) mappings across all column pairs
        final_name = self._prefixed("unique_target")
    
        join_conditions = []
        for i, ((key_cols, target_cols), tbl) in enumerate(
            zip(self.column_pairs, filter_tables)
        ):
            key_expr = (
                key_cols if isinstance(key_cols, str)
                else " || '-' || ".join(key_cols)
            ...
            join_conditions.append(f"""
                ({key_expr}, {target_expr}) IN (
                    SELECT computed_key, computed_target FROM {tbl}
                )
            """)
