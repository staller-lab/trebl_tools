...
class TreblPipeline:
    ...
    def _run_trebl_experiment_helper(
        ...
        for file_path in seq_files:
            ...
            map_order = (
                ["quality", "error_corrected"] if self.error_correction else ["quality"]
            )
            if not umi_object:
                # non-UMI workflow adds additional steps
                map_order = map_order + (
                    ["grouped", "thresholded", "designed"]
                    if not self.error_correction
                    else ["grouped", "thresholded", "designed"]
                )
            ...
            if umi_object:
                # Deduplication
                refined_map_suffix = (
                    "error_corrected" if self.error_correction else "quality"
                )
                ...
                if umi_deduplication == "simple":
                    ...
                    complex_df = pd.read_csv(
                        output_dir / f"{name_only}_directional_umi_counts.tsv", sep="\t"
                    )
                    complex_df["name"] = name_only
                    results.append(complex_df)

                simple_df = pd.read_csv(
                    output_dir / f"{name_only}_simple_umi_counts.tsv", sep="\t"
                )
                ...
                one_file_reads_per_UMI.to_csv(
                    output_dir / f"{name_only}_reads_per_umi.tsv", sep="\t"
                )

        # Merge results safely for both simple and complex dedup
        if umi_object:
            # Complex DF (directional/complex dedup)
            if results:  # there is directional/complex dedup
                ...
                complex_df = pd.DataFrame(
                    columns=[gene_col_name, count_col_name, "name"]
                )

            # Simple DF (simple dedup)
            simple_df = pd.concat(simple_results, ignore_index=True).rename(
                columns={"count": count_col_name}
            )

            # Create gene_col_name consistently
            if concat_gene:
                # concatenate multiple barcode columns (AD workflow)
                concat_cols = [bc.name for bc in bc_objects]
                simple_df[gene_col_name] = simple_df[concat_cols].agg("".join, axis=1)
            else:
                # single barcode (RT workflow)
                # pick first bc column as gene
                bc_col = bc_objects[0].name
                if gene_col_name not in simple_df.columns:
                    simple_df[gene_col_name] = simple_df[bc_col]

            # Merge simple + complex DFs
            if not complex_df.empty:
                ...
                # no complex dedup: return simple DF only
                merged = simple_df.copy()
                merged = merged.rename(
                    columns={count_col_name: f"{count_col_name}_simple"}
                )
    ...
    def trebl_experiment_analysis(
        ...
        for name, spec in experiments.items():
            ...
            if self.output_path:
                df.to_csv(
                    self.output_path / f"{name}_trebl_experiment_results.csv",
                    index=False,
                )
